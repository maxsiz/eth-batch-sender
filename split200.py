...
i=0
while len(receivers_list) >= 200:
    batch = receivers_list[i:i+200]
    batch_file = open(file1.split('.')[0]+'200_'+str(i)+'.csv', 'w')
    batch_file.writelines([s+'\n' for s in batch])
    batch_file.close()
    receivers_list = receivers_list[i+200:]
    i += 1
    print('File: {}, len()={}, len(receivers_list)={}'.format(batch_file, len(batch), len(receivers_list)))
    if len(receivers_list) < 200:
